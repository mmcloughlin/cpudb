...
func ParseCPUIDFiles(dir string) []*cpudb.CPU {
	filenames, err := CPUIDFiles(*src)
	if err != nil {
		log.Fatal(err)
	}

	cpus := make([]*cpudb.CPU, 0, len(filenames))
...
func CPUGoSyntax(cpu *cpudb.CPU) string {
	return strings.Replace(fmt.Sprintf("%#v", *cpu), "cpudb.", "", -1)
}

// Build Go source code that defines the given list of CPUs. Output not formatted.
func Build(cpus []*cpudb.CPU) []byte {
	w := new(bytes.Buffer)

	_, self, _, _ := runtime.Caller(0)
	fmt.Fprintf(w, "// Code generated by go run %s. DO NOT EDIT.\n\n", filepath.Base(self))
	fmt.Fprintf(w, "package cpudb\n\n")

	fmt.Fprintf(w, "// CPUs contains the CPU database.\n")
